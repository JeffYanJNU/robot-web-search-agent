...
AdditionType = Literal[
    "新注册企业", "存量企业新增机器人业务", "首次公开曝光",
    "已有企业新增产品", "系统首次发现",
]
EvidenceType = Literal[
    "registration", "product_launch", "mass_production", "funding", "delivery",
    "order", "new_business", "official_identity",
]
EXTRACTOR_PROMPT_VERSION = "2026-07-21.1"
...
字段：original_name、canonical_name、chinese_name、english_name、country、region_type、official_website、unified_social_credit_code、registration_date、robot_categories、representative_products、business_summary、discovery_signal、addition_type_hint、classification_evidence、evidence_date、robot_relevance、has_robot_product、has_commercial_progress、is_priority_category、field_evidence。
...
classification_evidence 用一句话写出支持分类的原文事实。field_evidence 逐项保存可直接核验的原文句子；evidence_type 只能是 registration、product_launch、mass_production、funding、delivery、order、new_business、official_identity；quote 必须是网页中的原句，不得改写；value 填被证明的日期、产品、金额或主体，没有则留空。representative_products 只填正文明确出现的产品或型号。
...
class DeepSeekCompanyExtractor:
    ...
    def extract(self, page: Page) -> list[ExtractedCompanyCandidate]:
        ...
        normalized_content = re.sub(r"\s+", " ", page.content).strip()
        for candidate in envelope.candidates:
            candidate.source_url = page.url
            candidate.field_evidence = [
                evidence
                for evidence in candidate.field_evidence
                if re.sub(r"\s+", " ", evidence.quote).strip() in normalized_content
            ]
